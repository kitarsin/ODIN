...
export function Dashboard({ onSelectLevel }: DashboardProps) {
  ...
  return (
    <div className="w-full h-full bg-background text-foreground flex flex-col overflow-hidden">
      {/* Top Navigation */}
      <nav className="h-16 border-b border-border backdrop-blur-md bg-card/80 shrink-0">
        <div className="h-full px-4 lg:px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 lg:gap-6">
            {/* XP Level */}
            <div className="flex items-center gap-2 px-3 lg:px-4 py-2 rounded-lg bg-secondary/50 border border-[#2979FF]/30">
              <Trophy className="w-4 h-4 lg:w-5 lg:h-5 text-[#2979FF]" />
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Level</span>
                <span className="font-semibold text-sm">{userLevel}</span>
              </div>
              <div className="w-px h-8 bg-border mx-2" />
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">XP</span>
                <span className="font-semibold text-sm text-[#2979FF]">{userXP}</span>
              </div>
            </div>

            {/* Streak Counter */}
            <div className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-lg bg-secondary/50 border border-[#FFAB00]/30">
              <Flame className="w-5 h-5 text-[#FFAB00]" />
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground">Streak</span>
                <span className="font-semibold text-sm text-[#FFAB00]">{streakDays} days</span>
              </div>
            </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
          <div className="max-w-5xl mx-auto">
            <div className="mb-8">
              <h2 className="text-2xl lg:text-3xl font-semibold mb-2 bg-gradient-to-r from-[#2979FF] to-[#00E676] bg-clip-text text-transparent">
                Mission Control
              </h2>
              <p className="text-muted-foreground text-sm lg:text-base">Select a level to begin your training</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              {mockLevels.map((level) => (
                <button
                  ...
                  className={`
                    relative p-4 lg:p-6 rounded-xl backdrop-blur-md transition-all duration-300
                    ${level.isUnlocked 
                ...
                >
                  ...
                  <div className="relative flex items-start justify-between">
                    <div className="flex-1 text-left">
                      ...
                      <h3 className="text-base lg:text-lg font-semibold mb-1">{level.title}</h3>
                      <p className="text-xs lg:text-sm text-muted-foreground mb-3">{level.description}</p>
                      <div className="flex items-center gap-2">
        ...
        {/* Leaderboard Sidebar */}
        <div className="hidden xl:block w-80 border-l border-border backdrop-blur-md bg-card/40 overflow-y-auto shrink-0">
          <div className="p-6">
            <div className="mb-6">
