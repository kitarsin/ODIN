...
export function Posttest() {
  const { user, completePosttest } = useAuth();
  const { isGameMode } = useTheme();
  const navigate = useNavigate();

  const STORAGE_KEY = `odin_posttest_${user?.id || 'guest'}`;
  const getSavedState = () => {
    try {
  ...
  // ── Check posttest already completed (redirect if so, but only on initial load)
  useEffect(() => {
    if (user?.posttestCompleted && phase === 'intro') {
      navigate('/dashboard', { replace: true });
    }
  }, [user?.posttestCompleted, phase, navigate]);
  ...
  const handleNext = async () => {
    ...
    if (isLast) {
      ...
      setSubmitError('');
      try {
        await completePosttest(responses.current);
      ...
      <div className={`min-h-screen flex items-center justify-center p-4 ${bg}`}>
        <div className={`w-full max-w-lg border p-8 shadow-2xl ${cardCls}`}>
          <h1 className={`text-xl font-semibold mb-3 ${text}`} style={isGameMode ? { fontFamily: 'var(--font-pixel)', fontSize: '16px' } : mono}>
            {isGameMode ? '> SYSTEM_EVALUATION' : 'Post Test Evaluation'}
          </h1>
          <p className={`text-sm mb-2 ${muted}`} style={pixel}>
            Complete this assessment to evaluate your learning and progress.
          </p>
          ...
          >
            {isGameMode ? '> BEGIN_' : 'Begin Post Test'}
          </Button>
        </div>
      </div>
    );
      ...
      <div className={`min-h-screen flex items-center justify-center p-4 ${bg}`}>
        <div className={`w-full max-w-md border p-10 shadow-2xl text-center ${cardCls}`}>
          ...
          <h1 className={`text-xl font-semibold mb-3 ${text}`} style={isGameMode ? { fontFamily: 'var(--font-pixel)', fontSize: '14px' } : mono}>
            {isGameMode ? '> EVALUATION_COMPLETE' : 'Thank you for participating!'}
          </h1>
      ...
      <div className={`w-full max-w-4xl mx-auto border shadow-2xl flex flex-col ${cardCls}`} style={{ minHeight: '80vh' }}>
        ...
        <div className={`flex items-center justify-between px-6 py-3 border-b ${isGameMode ? 'border-[#00ff41]/30' : 'border-border'}`}>
          ...
          <div className="flex gap-1.5">
            {PROBLEMS.map((_, i) => (
              <div key={i} className={`h-1.5 w-8 rounded-full transition-all ${i < questionIndex ? (isGameMode ? 'bg-[#00ff41]' : 'bg-primary')
        ...
        <div className="flex flex-col flex-1 p-6 gap-5">
          ...
          <div className="flex flex-col flex-1">
            <div className={`flex items-center justify-between px-3 py-1.5 rounded-t text-xs font-medium ${isGameMode
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              onClick={handleNext}
              disabled={phase === 'submitting'}
              className={`ml-auto flex items-center gap-2 font-semibold ${isGameMode
            ...
            >
              {phase === 'submitting'
                ...
                : isLast
                  ? (isGameMode ? '> SUBMIT_' : 'Submit Post Test')
          ...
          {/* Compile result */}
          {compileResult && (
            <div className={`rounded border text-sm ${passed
              ...
              <div className={`flex items-center gap-2 px-4 py-2 border-b font-medium ${passed
              ...
              {compileResult.actualOutput !== null && compileResult.actualOutput !== undefined && (
                <div className={`px-4 py-3 border-t text-xs ${passed ? isGameMode ? 'border-[#00ff41]/20' : 'border-green-500/20' : isGameMode ? 'border-red-500/20' : 'border-destructive/20'}`}>
                  <span className="font-medium style={pixel}">Output: </span>
